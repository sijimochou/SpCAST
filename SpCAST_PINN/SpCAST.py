#  This part of the code is modified based on https://github.com/KindXiaoming/pykan/blob/master/kan.
...
class GeneExpressionDataset(Dataset):
    def __init__(self, data, labels=None, coords=None):
        """
        Universal Gene Expression Dataset
        Can handle both single-cell and spatial transcriptomics data.

        Args:
        - data: Gene expression matrix (numpy.ndarray or torch.Tensor)
        - labels: Cell type labels (only for single-cell data; None for spatial transcriptomics)
        - coords: Spatial coordinates (only for spatial transcriptomics; None for single-cell data)
        """
        self.data = torch.tensor(data, dtype=torch.float32) \
            if not torch.is_tensor(data) else data
        self.labels = labels
        self.coords = coords
    ...
    def __getitem__(self, idx):
        ...
        c = None if self.coords is None else self.coords[idx]

        # Return structure:
        # If labels exist and coords exist, return (expr, label, coords)
        # If only labels exist, return (expr, label)
        # If only coords exist, return (expr, coords)
        if self.labels is not None and self.coords is not None:
            return x, y, c
...
class SpCAST(nn.Module):
    def __init__(self, input_dim, coord_dim, latent_dim1, latent_dim2,
                 num_classes, noise_scale=0.2, num=5, device=None):

        super(SpCAST, self).__init__()
        self.input_dim = input_dim          # Gene expression dimension
        self.coord_dim = coord_dim          # Coordinate dimension (2D/3D)
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, latent_dim1, bias=False),
            KANLayer(latent_dim1, latent_dim2, noise_scale=noise_scale, device=device, num=num),
        )

        # Coordinate feature encoder (Small MLP)
        self.coord_encoder = nn.Sequential(
            nn.Linear(coord_dim, 16, bias=False),
            nn.Tanh(),
            # nn.Linear(16, 16),
            # nn.ReLU()
        )

        # Classification branch (using expression features only)
        self.classifier = nn.Linear(latent_dim2, num_classes, bias=False)

        # Field prediction branch (fusing expression + coordinate features)
        self.field_regressor = nn.Linear(latent_dim2 + 16, 1)

        # Learnable PDE diffusion coefficient
        self.D_params = nn.Parameter(torch.ones(1))

        # Reconstruction branch (using expression features only)
        self.decoder = nn.Sequential(
            KANLayer(latent_dim2, input_dim, noise_scale=noise_scale, device=device, num=num),
        )

    def forward(self, x_expr, coords):
        """
        x_expr: Gene expression matrix   shape = [batch, input_dim]
        coords: Coordinate matrix        shape = [batch, coord_dim]
        """
        # 1) Encode expression
        expr_latent = self.encoder(x_expr)

        # 2) Encode coordinates
        coord_latent = self.coord_encoder(coords)

        # 3) Classification branch (Expression features)
        combined_class = self.classifier(expr_latent)

        # 4) Field prediction branch (Expression + Coordinates)
        field_input = torch.cat([expr_latent, coord_latent], dim=1)
        c_fields = self.field_regressor(field_input)

        # 5) Reconstruction branch
        reconstruct = self.decoder(expr_latent)

        return expr_latent, combined_class, reconstruct, c_fields
