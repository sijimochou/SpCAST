...
def prepare_data(scRNA_data, spRNA_data, batch_size=128, balance_classes=True):
    ...
    # Get spatial coordinates for spots
    coords = spRNA_data.obsm['spatial']

    source_dataset = GeneExpressionDataset(X_train, labels=y_train, coords=None)
    target_dataset = GeneExpressionDataset(X_test, labels=None, coords=coords)
...
def pde_residual_single(coords, c_field, D_param):
    """
    Args:
        coords: (batch, 2), requires_grad=True
        c_field: (batch, 1), scalar field values at coords
        D_param: (1,), diffusion coefficient
    """
    # First-order derivatives
    grads = autograd.grad(
        c_field, coords,
        grad_outputs=torch.ones_like(c_field),
        create_graph=True,
        retain_graph=True
    )[0]  # (batch, 2)

    # Second-order derivatives
    d2c_dx2 = autograd.grad(
        grads[:, 0], coords,
        grad_outputs=torch.ones_like(grads[:, 0]),
        create_graph=True,
        retain_graph=True
    )[0][:, 0]
    d2c_dy2 = autograd.grad(
        grads[:, 1], coords,
    ...
    R = -0.1 * c_field.squeeze()

    # PDE residual
    residual = D_param * laplacian_c + R
    loss = torch.mean(residual ** 2)
    return loss


def train_SpCAST(scRNA_data, spRNA_data,
                 latent_dim1=30, latent_dim2=20, n_epochs=10, loss_step=1,
                 batch_size=128, lr=0.001, gradient_clipping=5.0,
                 weight_decay=0.0001, verbose=True, random_seed=2025,
                 device=None, save_model=True, save_model_path='../model',
                 save_model_name='model_params.pth'):
    """
    Train the SpCAST model with single-cell and spatial RNA-seq data (坐标参与场预测).

    Args:
        scRNA_data: Single-cell RNA-seq AnnData object.
        spRNA_data: Spatial RNA-seq AnnData object.
        latent_dim1/latent_dim2: Latent space dimensions.
        n_epochs: Number of training epochs.
        loss_step: Interval (in epochs) for printing loss.
    ...
    Returns:
        prob_df: DataFrame of predicted probabilities for spRNA data.
    """
    ...
    # ===== Data preparation =====
    train_source_dataloader, train_target_dataloader, \
        test_source_dataloader, test_target_dataloader, \
        input_dim, num_classes, label_encoder = prepare_data(scRNA_data, spRNA_data, batch_size=batch_size)

    # Get coordinate dimension
    example_coords = next(iter(train_target_dataloader))[1]
    coord_dim = example_coords.shape[1]

    # ===== Initialize model =====
    model = SpCAST(input_dim=input_dim,
                   coord_dim=coord_dim,
    ...
    for epoch in tqdm(range(n_epochs), desc="Training"):
        ...
        for (source_data, source_label), (target_data, coords) in zip(cycle(train_source_dataloader),
            ...
            # Concatenate expression data (source + target)
            mixed_expr = torch.cat([source_data, target_data], dim=0).squeeze()

            # Concatenate coordinates (use zeros as dummy coordinates for source samples)
            dummy_coords = torch.zeros(source_data.size(0), coord_dim, device=device)
            mixed_coords = torch.cat([dummy_coords, coords], dim=0)

            # Forward pass
            combined, combined_class, reconstruct, c_fields = model(mixed_expr, mixed_coords)

            # PDE loss is only computed on the target part
            target_c_fields = c_fields[source_data.size(0):]
            loss_phys = pde_residual_single(coords, target_c_fields, model.D_params)

            # Split source/target latent representations
            source_combined, target_combined = torch.split(combined, [source_data.size(0), target_data.size(0)], dim=0)
            source_combined_class, target_combined_class = torch.split(combined_class,
                                                                       [source_data.size(0), target_data.size(0)],
                                                                       dim=0)
            ...
            recon_loss = recon_criterion(mixed_expr, reconstruct)

            total_loss = classification_loss + 0.03 * opt_loss + 0.01 * recon_loss + 0.001 * loss_phys
            ...
            total_phy_loss += loss_phys.item()

        if verbose and (epoch + 1) % loss_step == 0:
            train_accuracy = correct / total
            print(
                f'Epoch:{epoch + 1} MMD Loss:{total_opt_loss:.3f}  Class Loss:{total_classification_loss:.3f}  Recon loss:{total_recon_loss:.3f} Phy loss:{total_phy_loss:.3f} Train_Accuracy:{train_accuracy * 100:.2f}%')
    ...
    all_sp_preds, all_sc_preds = [], []
    all_sp_obsm, all_sc_obsm = [], []
    all_sp_reconstruct, all_sc_reconstruct = [], []
    all_sp_c_fields = []
    prb_matrix = []

    with torch.no_grad():
        # Test on spatial data
        for target_data, coords in tqdm(test_target_dataloader, desc="Testing spRNA data"):
            target_data = target_data.to(device)
            coords = coords.to(device)
            combined, combined_class, reconstruct, target_c_fields = model(target_data, coords)
            ...
            all_sp_c_fields.extend(target_c_fields.cpu().numpy())
        ...
        # Test on scRNA data
        for source_data, _ in tqdm(test_source_dataloader, desc="Testing scRNA data"):
            source_data = source_data.to(device)
            dummy_coords = torch.zeros(source_data.size(0), coord_dim, device=device)
            combined, combined_class, reconstruct, _ = model(source_data, dummy_coords)
    ...
    spRNA_data.obs['target_c_fields'] = all_sp_c_fields

    class_names = label_encoder.classes_

    prob_df = pd.DataFrame(data=prb_matrix, columns=class_names)
